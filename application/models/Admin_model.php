...
class Admin_model extends CI_Model {
    ...
    public function generate_qucode($id, $type)
    {
        ...
        $params['data'] = base_url().'extract/'.$type.'/view/'.md5($id);
        $params['level'] = 'H';
        $params['size'] = 8;
    ...
    // get_customer_info
    function get_extract_invoice($id)
    {
    ...
    {
        $this->db->select('i.*, p.name as item_name, p.details as pdetails, t.name as tax_name, t.rate as tax_rate');
        $this->db->from('invoice_items i');
        $this->db->join('products p', 'p.id = i.item_id', 'LEFT');
        $this->db->join('tax t', 't.id = i.tax_id', 'LEFT');
