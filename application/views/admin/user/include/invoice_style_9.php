            ...
            <table class="table">
                <thead class="pre_head2">
                    <tr class="pre_head_tr2 inv-pl30">
                        ...
                        <th class="border-0 font-weight-bold" style="color: <?php echo html_escape($color) ?>"><?php echo trans('hsn') ?></th>
                        <th class="border-0 font-weight-bold" style="color: <?php echo html_escape($color) ?>"><?php echo trans('quantity') ?></th>
                        <th class="border-0 font-weight-bold" style="color: <?php echo html_escape($color) ?>"><?php echo trans('unit') ?></th>
                        <th class="border-0 font-weight-bold" style="color: <?php echo html_escape($color) ?>"><?php echo trans('price') ?></th>
                        <th class="border-0 font-weight-bold" style="color: <?php echo html_escape($color) ?>"><?php echo trans('gst') ?></th>
                ...
                <tbody>
                    ...
                    <?php if (isset($page_title) && $page_title == 'Invoice Preview'): ?>
                        ...
                        <?php else: ?>
                            <?php for ($i=0; $i < $total_items; $i++) { ?>
                                <tr>
                                    <td width="25%">
                                        <?php $product_id = $this->session->userdata('product_ids')[$i] ?>
                                    ...
                                    <td><?php echo $this->session->userdata('hsn')[$i] ?></td>
                                    <td><?php echo $this->session->userdata('quantity')[$i] ?></td>
                                    <td><?php echo $this->session->userdata('unit')[$i] ?></td>
                                    <td><?php echo price_formatted_alt($this->session->userdata('price')[$i], $this->business->id, $currency_symbol) ?></td>
                                    <td><?php echo $this->session->userdata('tax_name')[$i] ?> - <?php echo $this->session->userdata('tax_rate')[$i] ?> %</td>
                                    <td>
                        ...
                        <?php else: ?>
                            <?php $i=1; foreach ($items as $item): ?>
                                <tr class="inv-pl30 t78">
                                    <td width="25%"><?php echo html_escape($item->item_name) ?> <br> <small><?php echo nl2br($item->details) ?></small></td>
                                    <td><?php echo html_escape($item->hsn) ?></td>
                                    ...
                                    <td><?php echo html_escape($item->tax_name) ?> - <?php echo html_escape($item->tax_rate) ?> %</td>
                    ...
                    <tr class="inv-pl30">
                        <td></td>
                        <td></td>
                        <td colspan="4" class="text-right"><strong><?php echo trans('sub-total') ?></strong></td>
                    ...
                    <?php if (!empty($discount)): ?>
                        <tr class="inv-pl30">
                            <td></td>
                            <td></td>
                            <td colspan="4" class="text-right"><strong><?php echo trans('discount') ?> <?php echo html_escape($discount) ?>%</strong></td>
                            <td><span>
                                <?php 
                                $total_discount = $sub_total * ($discount / 100);
                                $rprice = $sub_total - $total_discount;
                                echo price_formatted_alt($total_discount, $this->business->id, $currency_symbol) 
                                ?>
                                </span>
                            </td>
                        </tr>
                    ...
                    <?php if (!empty($total_items)): ?>
                        <?php for ($i=0; $i < $total_items; $i++) { ?>
                            <tr class="inv-pl30  t78 bg-blight">
                                <td></td>
                                <td></td>
                                <td colspan="4" class="text-right"><?php echo $this->session->userdata('tax_name')[$i] ?> - <?php echo $this->session->userdata('tax_rate')[$i] ?> %</td>
                                <td><span>
                                <?php 
                                $total_discount = $this->session->userdata('total_price')[$i] * (1 - $discount / 100) * $this->session->userdata('tax_rate')[$i] / 100;
                                echo price_formatted_alt($total_discount, $this->business->id, $currency_symbol) 
                                ?>
                                </span></td>
                            </tr>
                    ...
                    <?php $i=1; foreach ($items as $item): ?>
                        <tr class="inv-pl30 t78">
                            <td></td>
                            <td></td>
                            <td colspan="4" class="text-right"><strong><?php echo html_escape($item->tax_name) ?> - <?php echo html_escape($item->tax_rate) ?> %</strong></td>
                            <td><?php 
                                echo price_formatted_alt($item->total * (1 - $discount / 100) * $item->tax_rate / 100, $this->business->id, $currency_symbol);
                            ?></td>
                        </tr>
                    <?php $i++; endforeach ?>
                    ...
                    <tr class="inv-pl30">
                        <td></td>
                        <td></td>
                        <td colspan="4" class="text-right"><strong><?php echo trans('adjustment') ?></strong></td>
                        <td><span><?php echo price_formatted_alt($invoice->adjustment, $this->business->id, $currency_symbol) ?></span></td>
                    </tr>
                    ...
                    <tr class="inv-pl30">
                        <td></td>
                        <td></td>
                        <td colspan="4" class="text-right"><strong><?php echo trans('grand-total') ?></strong></td>
                    ...
                    <?php foreach (get_invoice_payments($invoice->id) as $payment): ?>
                        <tr class="inv-pl30 text-dark">
                            <td></td>
                            <td></td>
                            <td colspan="4" class="text-right" width="60%">
