        ...
        <section class="content p-0">
            ...
            <div class="extract-top-bar">
                ...
                <?php if($invoice->status == 2): ?>
                    <?php echo trans('amount-due') ?>: <?php echo price_formatted_alt('0', $invoice->biz_id, $currency_symbol) ?>
                <?php else: ?>
                    <?php echo trans('amount-due') ?>: <?php echo price_formatted_alt($invoice->grand_total - get_total_invoice_payments($invoice->id, $invoice->parent_id), $invoice->biz_id, $currency_symbol); ?>
                        ...
                        <div class="col-md-6">
                            ...
                            <?php if(check_package_limit('invoice-payments') == -1 && $invoice->type != 4): ?>
                                ...
                                <?php if ($user->paypal_payment == 1 || $user->stripe_payment == 1 || $user->razorpay_payment == 1): ?>
                                    <?php if ($invoice->status != 2): ?>
                                        <a href="<?php echo base_url('admin/payment/online/1/'.md5($invoice->id)) ?>" class="btn btn-info btn-rounded mr-5 <?php if(get_user_info() == FALSE){echo 'd-none';} ?>"><i class="fa fa-dollar"></i> <?php echo trans('pay-online') ?> </a>
                                    <?php endif ?>
                                <?php endif ?>
                            <?php endif ?>
                        </div>
