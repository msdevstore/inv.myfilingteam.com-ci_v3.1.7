...
(function($) {
  ...
  $(document).on('keyup', '#discount', function(e) {
    $('.discount').val(e.target.value);
  });

  $(document).on('keyup', '#adjustment', function(e) {
    $('.adjustment').val(e.target.value);
  });
  ...
  $(document).on('change', ".report_types", function() {
    ...
    $(document).on('click', ".inv-item", function() {
      ...
      $.post(url, { data: 'value', 'csrf_test_name': csrf_token }, function(json) {
        if(json.st == 1){
          ...
          $('#currency_wrapper').val(json.currency);
        }
      }, 'json' );
      return false;
    });
    ...
    $(document).on('change', ".tax_id", function() {
      ...
      if(Id != ''){
        ...
        $.post(url,{ data: 'value', 'csrf_test_name': csrf_token },function(data){
          const tax = JSON.parse(data);
          $('#tax_'+type).val(tax.rate);
          $('#tax_name_'+type).val(tax.name);
        }
        );
      } 
    });
    ...
    $(function(){
      $(document).on('submit', ".record_payment_form", function() {

        $(this).find(':input[type=submit]').prop('disabled', true);

        $.post($('.record_payment_form').attr('action'), $('.record_payment_form').serialize(), function(json){
